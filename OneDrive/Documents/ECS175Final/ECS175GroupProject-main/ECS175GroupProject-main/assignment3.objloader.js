...
class OBJLoader {
    ...
    load( gl ) {
        ...
        let sizeOfVertex = vertex_positions.length/3
        for(let i = 0; i < sizeOfVertex; i++){
            vertex_data.push(
                vertex_positions[3*i],
                vertex_positions[3*i+1],
                vertex_positions[3*i+2]
            )
            vertex_data.push(
                vertex_normals[3*i],
            ...
            if (material.hasTexture()) {
                vertex_data.push(
                    vertex_tangents[3 * i],
                    vertex_tangents[3 * i + 1],
                    vertex_tangents[3 * i + 2]
                );
                vertex_data.push(
                    vertex_texture_coords[2 * i],
                    vertex_texture_coords[2 * i + 1]
                );
            }
        }   
        if (material.hasTexture()) {
